...
poisson.MixMod <- function(formula, data=NULL, W, burnin, n.sample, thin=1, trends=NULL, changepoint=NULL, knots=NULL, prior.mean.beta=NULL, prior.var.beta=NULL,
                           prior.mean.gamma=NULL, prior.var.gamma=NULL, prior.lambda=NULL, prior.tau2=NULL, Nchains=4, verbose=TRUE)
{
  #------------------------------------------------------------------------------------
  ...
  if(is.null(prior.var.gamma)) prior.var.gamma <- rep(1000, params.selected)
  ...
  if(is.null(prior.tau2)) prior.tau2 <- c(1, 0.1)
  
  if(length(prior.mean.beta) != p) stop("the prior mean for beta is the wrong length.", call.=FALSE)    
  if(!is.numeric(prior.mean.beta)) stop("the prior mean for beta is not numeric.", call.=FALSE)    
  if(sum(is.na(prior.mean.beta)) != 0) stop("the prior mean for beta is missing.", call.=FALSE)  
  
  if(length(prior.var.beta) != p) stop("the prior variance for beta is the wrong length.", call.=FALSE)    
  if(!is.numeric(prior.var.beta)) stop("the prior variance for beta is not numeric.", call.=FALSE)    
  if(sum(is.na(prior.var.beta)) != 0) stop("the prior variance for beta is missing.", call.=FALSE)    
  if(min(prior.var.beta) <= 0) stop("the prior variance for beta is less than zero", call.=FALSE)
  ...
  #------------------------------------------------------------------------------------
  # Keep track of the additional positions of the selected gamma parameters of the
  # CP/CT and MD/MI trends
  #------------------------------------------------------------------------------------
  ...
  for(j in 1:n.sample)
  {
    ...
    for(r in 1:n.beta.block)
    {
      ...
      beta.linpred <- linpredcompute(X.standardised, N.all, p, beta, Nchains)
      proposal.linpred <- linpredcompute(X.standardised, N.all, p, proposal.beta, Nchains)
      
      prob <- poissonbetablockupdate(N.all, beta, proposal.beta, beta.linpred, proposal.linpred, offset.temp, Y, prior.mean.beta, prior.var.beta, Nchains, temps, p)
    ...
    regression.mat <- linpredcompute(X.standardised, N.all, p, beta, Nchains)
    #------------------------------------------------------------------------------------
    ...
    for (i in Trends.id)
    {
      gamma.proposal <- gammaproposal(Nchains, gamma[gamma.pos[i], ], proposal.sd.gamma[gamma.pos[i], ], prior.var.trends[i], W.areas[which(Trends.chosen.names %in% Trends.pos[i])[1], ], i, knots)
      ...
      gamma.list <- poissongammaupdate(N.all, gamma[gamma.pos[i], ], gamma.proposal, offset.temp, offset.proposal, Y, prior.mean.trends[i], prior.var.trends[i], Nchains, temps)
    ...
    w.list <- poissonwupdate(K, N, w.perm, offset.temp, offset.proposal, w.proposal, Y.mat, lambda, Nchains, temps, begin.chain, beg.reg.chain, N.trends)
    ...
    phi.list <- poissonphiupdate(W.triplet, W.begfin, W.triplet.sum, K, N, phi, offset.temp, Y.mat, tau2, rho, Nchains, temps, proposal.sd.phi, beg.reg.chain)
    if(!all(phi.list[[2]] == 0))
    {
    ...
    #------------------------------------------------------------------------------------
    # Samples from tau2 (no temperature levels as Gibbs sampled)
    #------------------------------------------------------------------------------------
    tau2.temp <- tau2quadform(W.triplet, W.triplet.sum, W.n.triplet, K, phi, phi, rho, Nchains)
    tau2 <- tau2compute(tau2.temp, tau2.shape, prior.tau2[2], Nchains)
    #------------------------------------------------------------------------------------
    # Samples from rho
    #------------------------------------------------------------------------------------
    rho.temp1 <- rhoquadformcompute(W.triplet, W.triplet.sum, W.n.triplet, K, Nchains, phi, rho, tau2)
    proposal.rho <- suppressWarnings(rtrunc(n = Nchains, spec = "norm", a = 0, b = 0.99, mean = rho, sd = proposal.sd.rho))
    rho.temp2 <- rhoquadformcompute(W.triplet, W.triplet.sum, W.n.triplet, K, Nchains, phi, proposal.rho, tau2)
    ...
    accept.swap <- poissoncouplingAllupdate(N.all, K, p, w.chain.mat, offset.temp, beta, gamma, lambda, phi, rho, tau2, W.triplet.sum, W.triplet,
                                            W.begfin, Y, prior.mean.beta, prior.var.beta, prior.mean.trends, prior.var.trends, prior.lambda, prior.tau2,
                                            swap, temps, begin.chain, N.trends, Trends.sel)
    ...
    if(accept.swap == 1)
    {
      ...
      w.chain.mat <- matrix(aperm(w, c(1, 3, 2)), nrow = K * Nchains, ncol = N.trends) # necessary?
  ...
  if(number.cts>0)
  {
    for(r in 1:p)
    {
      if(X.indicator[r]==1)
      {
        samples.beta.orig[ ,r] <- samples.beta[ ,r,chain.sel] / X.sd[r]
      }else if(X.indicator[r]==2 & p>1)
      {
        X.transformed <- which(X.indicator==1)
        samples.temp <- as.matrix(samples.beta[,X.transformed, chain.sel])
        ...
        samples.beta.orig[,r] <- samples.beta[,r,chain.sel] - intercept.adjustment
      }else
      {
      }
    }
  ...
  accept.w <- 100 * accept.w.all[, accept.blocks.num[,1]] / accept.w.all[,accept.blocks.den[,1]]
  accept.w <- accept.w[, chain.sel]
  accept.phis <- 100 * accept.phis.all[, accept.blocks.num[,1]] / accept.phis.all[,accept.blocks.den[,1]]
  accept.phis <- accept.phis[,chain.sel]
  
  accept.final <- c(accept.beta, accept.gammas, accept.rho, accept.w, accept.phis)
  names(accept.final) <- c("beta", paste("gamma.", Trends.pos[Trends.id], sep=""), "rho", paste("w.", 1:K, sep=""), paste("phi.", 1:K, sep=""))
  #------------------------------------------------------------------------------------
  ...
  #------------------------------------------------------------------------------------
  modelfit <- c(DIC[chain.sel], p.d[chain.sel], WAIC[chain.sel], p.w[chain.sel], LMPL[chain.sel])
  names(modelfit) <- c("DIC", "p.d", "WAIC", "p.w", "LMPL")
  
  samples <- list(beta=mcmc(samples.beta.orig), gamma=mcmc(samples.gamma[,,,chain.sel]), lambda=mcmc(samples.lambda[,,chain.sel]),
                  tau2=mcmc(samples.tau2[,,chain.sel]), rho=mcmc(samples.rho[,,chain.sel]), w=samples.w[,,,chain.sel], phi=mcmc(samples.phi[,,chain.sel]),
                  fitted=mcmc(samples.fitted[,,chain.sel]))
  
  model.string <- c("Likelihood model - poisson (log link function)")
  results <- list(summary.results=summary.results, samples=samples, fitted.values=fitted.values, trends=trends, trend.probs=trend.probs, residuals=residuals,
                  modelfit=modelfit, accept=accept.final, localised.structure=NULL, formula=formula, model=model.string, coupled=100*accept.couple[1]/accept.couple[2])
